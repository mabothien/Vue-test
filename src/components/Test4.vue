<template>
  <div class="table-wrap coltype" style="width: 50%">
    ...
    <table class="table ui-accordion">
      ...
      <tbody>
        <tr class="head" v-for="row in filteredList" :key="row.id">
          <td>{{row.order}}</td>
          <td>{{row.type}}</td>
          <td>{{row.question}}</td>
        </tr>
    ...
    filteredList() {
      ...
      if (self.search) {
        result = result.filter(
          item =>
            item.order.toString().includes(self.search) ||
            item.type.includes(self.search) ||
            item.question.includes(self.search)
        );
        return result;
      } else {
        return result;
