<template>
  <div class="table-wrap coltype" id="vue-test" style="width: 50%">
    <table class="table ui-accordion">
      ...
      <tbody>
        <template v-for="(row, index) in data.rows">
          <tr class="head" :key="row.title.order" v-on:click="toggle(index)">
          ...
          <tr
            class="body"
            v-bind:class="{ active: opened.includes(index)}"
...
export default {
  ...
  methods: {
    // Toggle collapse show/hide detail row
    toggle(id) {
